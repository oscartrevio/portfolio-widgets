    ...
    <div className="relative flex h-24 flex-grow-0 items-center justify-between overflow-hidden rounded-3xl p-4 text-white-a12 will-change-auto">
      ...
      <div className="z-30 flex w-full items-center justify-between">
        ...
        <div className="flex h-full flex-shrink-0 items-center justify-end gap-3">
          <ControlButton onClick={handlePreviousTrack} icon={<FaBackward className="mr-0.5" />} />
          <ControlButton
            onClick={handlePausePlay}
            icon={paused ? <FaPlay className="ml-1 text-2xl" /> : <FaPause className="text-2xl" />}
            progress={progress}
          />
          <ControlButton onClick={handleNextTrack} icon={<FaForward className="ml-0.5" />} />
        </div>
      </div>
    </div>
  );
...
function ControlButton({ onClick, icon, progress }: ControlButtonProps) {
  return (
    <motion.button
      type="button"
      onClick={onClick}
      className={cn(progress ? "size-12" : "size-9", "relative flex items-center justify-center overflow-hidden rounded-full text-center")}
      whileTap={{ scale: 0.9 }}
    >
      <div className="z-10">{icon}</div>
      <div className="absolute inset-0 z-0 h-full w-full bg-[#C2C2C2] opacity-45 mix-blend-overlay" />
      {progress !== undefined && (
        <svg className="-rotate-90 absolute top-0 left-0 h-full w-full" viewBox="0 0 100 100">
          <title>Progess Bar</title>
          <circle cx="50" cy="50" r="47" fill="transparent" strokeWidth="6" className="stroke-[#C2C2C2] opacity-45 mix-blend-overlay" />
          <circle
            cx="50"
            cy="50"
            r="47"
